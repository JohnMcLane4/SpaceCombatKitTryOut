...
namespace VSX.UniversalVehicleCombat.Radar
{
    ...
    public class HUDTargetBoxes : HUDComponent
    {
        ...
        void ShowTargetBox(Tracker tracker, Trackable trackable, HUDTargetBox targetBox)
        {
            ...
            for (int i = 0; i < tracker.TargetLeaders.Count; ++i)
            {
                if (tracker.TargetLeaders[i].Target != null && tracker.TargetLeaders[i].Target == trackable)
                {
                    ...
                    {
                        Vector3 viewportPos = HUDTargetBoxesFunctions.GetViewportPosition(tracker.TargetLeaders[i].LeadTargetPosition, hudCamera, true);

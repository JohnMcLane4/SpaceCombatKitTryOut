...
namespace VSX.UniversalVehicleCombat.Radar
{
    ...
    public class Tracker : ModuleManager, IHUDCameraUser
    {
        ...
        [HideInInspector]
        public TargetLeaderEventHandler onTargetLeaderAdded;

        [HideInInspector]
        public TargetLeaderEventHandler onTargetLeaderRemoved;
        ...
        protected virtual void Start()
        {
            ...
            // Store target leaders in hierarchy
            if (getComponentReferencesInHierarchy)
            {
                TargetLeader[] targetLeaders = transform.GetComponentsInChildren<TargetLeader>();
                foreach (TargetLeader targetLeader in targetLeaders)
                {
                    AddTargetLeader(targetLeader);
                }
            }
        }
        ...
        public virtual void AddTargetLeader(TargetLeader targetLeader)
        {
            ...
            {
                if (targetLeaders.IndexOf(targetLeader) == -1)
                {
                    targetLeaders.Add(targetLeader);
                    onTargetLeaderAdded.Invoke(targetLeader);
                }
            }
        }
        ...
        public virtual void RemoveTargetLeader(TargetLeader targetLeader)
        {
            if (targetLeader != null)
            {
                if (targetLeaders.IndexOf(targetLeader) != -1)
                {
                    targetLeaders.RemoveAt(targetLeaders.IndexOf(targetLeader));
                    onTargetLeaderAdded.Invoke(targetLeader);
                }
            }
        }
        ...
        protected override void OnModuleMounted(Module module)
        {
            ...
            // Store target leader
            AddTargetLeader(module.GetComponent<TargetLeader>());
        ...
        protected override void OnModuleUnmounted(Module module)
        {
            ...
            // Remove target leader
            RemoveTargetLeader(module.GetComponent<TargetLeader>());
